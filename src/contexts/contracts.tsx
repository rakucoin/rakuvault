...
export const ContractProvider = ({ children = null as any }) => {
	...
	const [stakingContract, setStakingContract] = useState<Contract>(
		new defaultWeb3.eth.Contract(config.stakingAbi as any, config.stakingAddress),
	);

	useEffect(() => {
		const web3Obj = new Web3(library?.provider || config.providerUrl);
		setWeb3(web3Obj);
		setStakingContract(new web3Obj.eth.Contract(config.stakingAbi as any, config.stakingAddress));
	}, [library]);
	...
	return (
		<ContractContext.Provider
			value={{
				web3,
				stakingContract: { contract: stakingContract, address: config.stakingAddress },
			}}
		>
			{children}
		</ContractContext.Provider>
